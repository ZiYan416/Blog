...
export default function EditPostPage({ params }: EditPostPageProps) {
  ...
  const handleUpdate = useCallback(async (published: boolean) => {
    if (!title) {
      toast({
        title: "标题不能为空",
        variant: "destructive",
  ...
  }, [title, tags, content, availableTags, slug, coverImage, originalSlug, toast, router])

  // 监听导航栏发布按钮事件
  useEffect(() => {
    const handleNavbarPublish = () => handleUpdate(true)
