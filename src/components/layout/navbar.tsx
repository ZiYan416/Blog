...
export function Navbar({ user: initialUser }: { user?: SupabaseUser | null }) {
  ...
  return (
    <>
      <header className={cn(
        "fixed top-0 left-0 right-0 z-50 transition-all duration-300 ease-in-out overflow-hidden select-none transform-gpu",
        !isNavbarVisible && "h-0 border-none bg-transparent",
        isNavbarVisible && isTransparent
          ? "h-16 bg-gradient-to-b from-black/50 via-black/20 to-transparent border-transparent"
          : isNavbarVisible && "h-16 border-b border-black/5 dark:border-white/5 bg-white/70 dark:bg-black/70 backdrop-blur-xl shadow-sm"
      )}>
        <div className={cn(
