...
```text
blog/YYYY/MM/<publicId>
```

已保存文章使用只读唯一数字标识 `public_id` 作为目录名。尚未首次保存的新文章没有数据库
编号，上传时回退到 `blog/YYYY/MM`；文件名仍使用 UUID 保证唯一。显式目录提示只接受有限
长度的 ASCII 字母、数字、下划线、连字符和正斜杠分段；绝对路径、反斜杠、控制字符、
`.`、`..` 和编码绕过都会被拒绝。
